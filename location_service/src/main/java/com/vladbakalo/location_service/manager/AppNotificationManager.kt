...
class AppNotificationManager constructor(val context: Context) {
    ...
    fun sendAlarmNotification(locationAlarmDb: LocationAlarmDb, alarmDistanceDb: AlarmDistanceDb) {
        val builder =
            NotificationCompat.Builder(context, ENotificationChannel.ALARM_NOTIFY.channelId)
                ...
                .setContentTitle(
                    String.format(context.getString(R.string.alarm_notification_tittle_distance),
                        locationAlarmDb.name, alarmDistanceDb.notifyDistanceMeters))
                .setContentText(locationAlarmDb.note)
                .setShowWhen(true)
        ...
        notificationManager.notify(getLocationAlarmNotifyId(locationAlarmDb), builder.build())
    }

    fun removeAlarmNotification(locationAlarmDb: LocationAlarmDb){
        notificationManager.cancel(getLocationAlarmNotifyId(locationAlarmDb))
    }
    ...
    private fun getLocationAlarmNotifyId(locationAlarmDb: LocationAlarmDb): Int{
        return (locationAlarmDb.id * 100).toInt()
    }
